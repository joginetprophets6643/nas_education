...
async function createSidebarStates(data) {
  ...
  await data.map((state) => {
    ...
    const filteredDistrict = district_data.filter((district) => {
      if (district.udise_state_code === state.udise_state_code) return district
    })
...
async function setScreen(screen_type = 'information', load_data = true) {
  ...
  const active_state = JSON.parse(sessionStorage.getItem('activeState'))
...
function updateData(data) {
  if (typeof data !== 'undefined') {
    ...
    if (screenType === 'participation') {
      ...
      if (
        data[0].district_id == undefined &&
        data[0].state_id != undefined &&
        data.length > 1
      ) {
        temp_data = JSON.parse(
          sessionStorage.getItem('state_participation_cards'),
        )
    ...
    if (screenType === 'learning') {
      ...
      data.forEach((lo) => {
        ...
        if (lo.language === 'language') {
          ...
          if (classType === 'all') {
            $('.language_lo' + lo.grade + '_class' + classType).append(
        ...
        if (lo.language === 'sci') {
          ...
          if (classType === 'all') {
            $('.science_lo' + lo.grade + '_class' + classType).append(
              getOutcomeRow(lo, classStyle),
            )
          } else {
        ...
        if (lo.language === 'sst') {
          ...
          if (classType === 'all') {
            $('.social_lo' + lo.grade + '_class' + classType).append(
              getOutcomeRow(lo, classStyle),
            )
          } else {
        ...
        if (lo.language === 'eng') {
          ...
          if (classType === 'all') {
            $('.english_lo' + lo.grade + '_class' + classType).append(
              getOutcomeRow(lo, classStyle),
            )
          } else {
    ...
    if (screenType === 'feedback') {
      ...
      data.forEach((fb) => {
        ...
        if (fb.level === 'pq') {
          ...
          if (current_demography === 'national') {
            pqChart =
              ...
              capitalizeFirstLetter(capitalizeFirstLetter(fb.question_desc)) +
              '</p></div></div>'
          } else {
            pqChart =
              '<div class="col-lg-3 mb-15"><div class="student-fbcard light-blue-bg"><div class="progressbar-circle-sm progressbar-blue"><div class="progress" data-percentage="' +
        ...
        if (fb.level === 'pq2') {
          ...
          $(
            '#feedback' + current_demography + '_pq2_' + countPq2 + '_class3',
          ).html(pq2)
          countPq2 += 1
        }
        if (fb.level === 'pq3') {
          ...
          if (current_demography === '' || current_demography === 'state') {
            pq3 =
              ...
              '</p></div>'
            $(
              '#feedback' + current_demography + '_pq3_' + countPq3 + '_class3',
            ).html(pq3)
      ...
      data.forEach((ach, index) => {
        if (index % 6 === 0) {
          if (classType == 'all') {
            ...
            achievement_data +=
              '</tbody></table></div></div><br>' + direction_gender_header
          }
        } else {
          if (index === 0) {
...
function getOutcomeRow(lo, classStyle) {
  let row = ''
  if (selected_geography === 'district') {
    $('.learning-los-head').html(
      'Performance of the district in achieving learning Outcomes (Los)',
    )
    row =
      '<tr class="' +
      classStyle +
    ...
    $('.learning-los-head').html(
      'Performance of the State/UT in achieving learning Outcomes (LOs)',
    )
    row =
      '<tr class="' +
      classStyle +
  ...
  try {
    ...
    sections.forEach((section) => {
      graphs.forEach((sub) => {
        ...
        if (classType !== 'all') {
          where =
            section + sub + current_demography + 'BarGraph_class' + classType
        } else {
          where =
            section +
            sub +
        ...
        demographics.forEach((demo, index) => {
          if (current_demography === '' || current_demography === 'national') {
            if (classType !== 'all') {
              ...
              if (!empty) {
                cardData = Math.round(
                  data[subjects_short_codes[sub.toLowerCase()]]['cards'][demo],
                )
              }
          ...
          if (current_demography === 'state') {
            if (classType !== 'all') {
              const item = {
                ...
                y: !empty
                  ? Math.round(
                      data[subjects_short_codes[sub.toLowerCase()]]['cards'][
                        demo
                      ],
                    )
                  : 0,
    ...
    if (current_demography === 'state') {
      ...
      if (classType !== 'all') {
        ...
        $('#state_district_table' + all_data.grade + '_class' + classType).html(
          row,
        )
      }
    }
...
async function createInformationScreen(data) {
  ...
  if (selected_geography === 'national') {
    ...
    $('#' + prefix + 'area_class3').html(
      parseInt(dataToShow.total_district_area)
        ? parseInt(dataToShow.total_district_area).toLocaleString('en-IN') +
            ' sq. km.'
        : '-',
    )
    $('#' + prefix + 'population_class3').html(
  ...
  $('#' + prefix + 'density_class3').html(
    parseInt(dataToShow.density_of_population)
      ? parseFloat(dataToShow.density_of_population).toLocaleString('en-IN') +
          ' per sq. km'
      : '-',
  )
  ...
  if (selected_geography === 'district') {
    ...
    $('#' + prefix + 'state_teacher_class3').html(
      Math.round(dataToShow.teacher_state_govt_schools).toLocaleString('en-IN'),
    )
    ...
    $('#' + prefix + 'govt_aided_teacher_class3').html(
      Math.round(dataToShow.teacher_govt_aided_schools).toLocaleString('en-IN'),
    )
...
function createDognutDataAndChart(data, subjects, grade) {
  ...
  Object.keys(data).forEach((key, index) => {
    ...
    if (classType === 'all') {
      where =
        'doghnut_' + key + grade + selected_geography + '_class' + classType
    } else {
...
function createCumulativeCardsForPerformance(data) {
  ...
  Object.keys(cumulative_subject_count).forEach((key) => {
    ...
    demographics.forEach((demo, index) => {
      if (selected_geography !== 'state') {
        ...
        if (selected_geography === 'district') {
          ...
          val = Math.round(
            cumulative_subject_count[subject][demo] / divisor[subject],
          )
        }
        if (selected_geography === 'national') {
          val = Math.round(
    ...
    if (selected_geography === 'state') {
      const card_chart_where =
        screenType + selected_geography + '_' + key + '_class' + classType
      const final_state_chart_data = [
        {
          data: state_chart_data,
        },
...
function setModalHeader(data, sub, legend) {
  ...
  if (data.grade == 3) {
    ...
    if (legend == 'cards') {
      $('#data-header').html(
        'Performance of States/UTs in Class V : ' + subject[sub].toUpperCase(),
      )
    } else {
      $('#data-header').html(
        'Performance of States/UTs by ' +
    ...
    if (legend == 'cards') {
      $('#data-header').html(
        'Performance of States/UTs in Class X : ' + subject[sub].toUpperCase(),
      )
    } else {
      $('#data-header').html(
        'Performance of States/UTs by ' +
...
function generateGlimpsesTable(data, legend, where) {
  ...
  data.forEach((actual_data) => {
    ...
    state_name =
      current_state.length != 0 ? current_state[0].state_name : 'National'
    if (legend == 'cards') {
      innerHtml += `<tr>
                  <td>${state_name}</td>
                  <td>${
                    actual_data['ss'] != 0 ? Math.round(actual_data['ss']) : '-'
                  }</td>
                  <td>${
                    actual_data['se'] != 0
                      ? (
                          Math.round(parseFloat(actual_data['se']) * 100) / 100
                        ).toFixed(1)
                      : '-'
                  }</td>`
      ...
      innerHtml += `<tr>
                  <td>${state_name}</td>
                  <td>${
                    actual_data['girls_ss'] != 0
                      ? Math.round(actual_data['girls_ss'])
                      : '-'
                  }</td>
                  <td>${
                    actual_data['boys_ss'] != 0
      ...
      innerHtml += `<tr>
                  <td>${state_name}</td>
                  <td>${
                    actual_data['rural_ss'] != 0
                      ? Math.round(actual_data['rural_ss'])
                      : '-'
                  }</td>
                  <td>${
                    actual_data['urban_ss'] != 0
      ...
      innerHtml += `<tr>
                  <td>${state_name}</td>
                  <td>${
                    actual_data['govt_ss'] != 0
                      ? Math.round(actual_data['govt_ss'])
                      : '-'
                  }</td>
                  <td>${
                    actual_data['govt_aided_ss'] != 0
      ...
      innerHtml += `<td>${
        actual_data['pvt_ss'] != 0 ? Math.round(actual_data['pvt_ss']) : '-'
      }</td>`

      innerHtml += generateIndictor(actual_data.pvt_category)

      innerHtml += `<td>${
        actual_data['central_govt_ss'] != 0
      ...
      innerHtml += `<td>${
        actual_data['st_ss'] != 0 ? Math.round(actual_data['st_ss']) : '-'
      }</td>`

      innerHtml += generateIndictor(actual_data.st_category)

      innerHtml += `<td>${
        actual_data['obc_ss'] != 0 ? Math.round(actual_data['obc_ss']) : '-'
      }</td>`
